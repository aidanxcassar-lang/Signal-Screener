...
function renderHub({ rows, sectors }) {
  const title = 'Crypto Research: Top Picks by Sector, Scored on Live Data | Signal Screener';
  const desc = `Researched reports on ${rows.length} crypto assets across ${Object.keys(sectors).length} sectors. `
             + `What each one does, why it stands out, what to watch, and a score built from live public data.`;
  ...
  : markSVG(r.coin.slug, 26)}</span><a href="/coins/${esc(r.coin.slug)}">${esc(r.coin.name)}</a>
            ...
            <td class="c-mc" data-usd="${r.fig.raw.marketCap||''}">${fmtUSD(r.fig.raw.marketCap)}</td>
          </tr>`).join('')}</tbody>
      </table>
    </section>`;
  }).join('');
...
<h1>Top picks by sector</h1>
<p class="lede">A short list of assets worth understanding, one sector at a time. Each has a written report explaining what it does, why it stands out and what to watch, plus a score built from live public data.</p>
