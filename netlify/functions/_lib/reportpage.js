...
import { markSVG, LOGOS, COINS as ALL_COINS } from './coins.js';
import { header, footer, BASE_CSS, CHROME_JS } from './chrome.js';
...
const ALL_SLUGS = Object.fromEntries(Object.keys(ALL_COINS).map(k => [k, ALL_COINS[k].name]));
...
<h2>Key figures</h2>
<div class="facts">
  <div class="fact"><div class="fact-l">Market cap</div><div class="fact-v" data-usd="${raw.marketCap||''}">${fmtUSD(raw.marketCap)}</div></div>
  ${raw.fdv ? `<div class="fact"><div class="fact-l">Fully diluted</div><div class="fact-v" data-usd="${raw.fdv||''}">${fmtUSD(raw.fdv)}</div></div>` : ''}
  ${fdvRatio ? `<div class="fact"><div class="fact-l">FDV / cap</div><div class="fact-v">${fdvRatio.toFixed(2)}x</div></div>` : ''}
  ${raw.volume24h ? `<div class="fact"><div class="fact-l">24h volume</div><div class="fact-v" data-usd="${raw.volume24h||''}">${fmtUSD(raw.volume24h)}</div></div>` : ''}
...
<h2>Other reports</h2>
<div class="peers other-reports">${Object.keys(ALL_SLUGS).filter(k=>k!==coin.slug).slice(0,7)
  .map(k=>`<a class="peer" href="/coins/${k}">${esc(ALL_SLUGS[k])}</a>`).join('')}</div>
