...
export default async () => {
  ...
  for (const slug of Object.keys(COINS)) {
    ...
    try {
      ...
      // Set once and never moved: the whole point is measuring from publication.
      const baselinePrice = prev?.baselinePrice ?? coin.publishedPrice ?? raw.price ?? null;
      const baselineAt = prev?.baselineAt ?? (Date.parse(coin.publishedISO) || Date.now());
