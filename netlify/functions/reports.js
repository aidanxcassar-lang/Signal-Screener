  ...
  if (u.searchParams.get('format') === 'json') {
    ...
    for (let i = 0; i < slugs.length; i++) {
      ...
      out.push({
        ...
        // Fall back to the published price when no live price arrived, so the row reads
        // "new" rather than a bare dash on a provider hiccup.
        sincePublished: (() => {
          const base = fig.baselinePrice ?? c.publishedPrice ?? null;
          const now = fig.price ?? fig.raw?.price ?? base;
